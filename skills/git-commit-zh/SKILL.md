---
name: git-commit-zh
description: Generate one precise Chinese Git commit message from the complete working-tree status and diff. Create one local commit when the user explicitly requests an automatic commit or clearly asks to commit the current changes. Always return the final message when changes exist. Never push.
---

# Chinese Git Commit Message

Inspect the complete repository state and generate exactly one Chinese commit message. Create one local commit only in explicit commit mode. Never push.
...
Always use `<type>(<scope>): <subject>`. Keep `<type>` and `<scope>` in English. Choose the narrowest stable English module, component, or affected-area scope supported by the diff; use `workspace` only when no narrower scope is supported. A scope may contain only lowercase ASCII letters, digits, and hyphens.
...
- Write the subject, bullets, and detail paragraph in Chinese. Keep the conventional English type token unchanged.
...
- Avoid undefined jargon, including `链路`, `闭环`, `沉淀`, `抓手`, `护栏`, `赋能`, `编排`, `对齐`, and `打通`, unless the term is the exact name of a defined technical concept.
- Avoid filler, unsupported motivation, credentials, and private paths.
